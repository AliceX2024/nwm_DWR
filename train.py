...
def main(args):
    ...
    ablation_mode = rw_cfg.get('ablation_mode', None)
        ...
        for data_iter_step, (x, y, rel_t) in enumerate(loader):
            ...
            with torch.amp.autocast('cuda', enabled=bfloat_enable, dtype=torch.bfloat16):
                ...
                if enable_reweight and not is_warmup:
                    ...
                    # [Ablation] random / fixed / original
                    if ablation_mode == 'random':
                        batch_weights = torch.rand(curr_features.shape[0], device=device)
                        batch_weights = batch_weights / (batch_weights.mean() + 1e-12)
                    elif ablation_mode == 'fixed':
                        batch_weights = torch.ones(curr_features.shape[0], device=device)
                    elif rw_cfg.get('use_spacetime', False):
                        # 提取观测特征: 历史帧最后时刻的特征
                    ...
                    if rank == 0 and train_steps % rw_cfg.get('debug_print_freq', 100) == 0:
                         with torch.no_grad():
                            ...
                            logger.info("-" * 40)
                # =========================================================
                #elif enable_reweight:
                    # warmup 期间或显式关闭 reweight 时，保持统一权重
                    #batch_weights = torch.ones(x_start.shape[0], device=device)
