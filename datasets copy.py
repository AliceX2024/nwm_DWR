...
class BaseDataset(Dataset):
    ...
    def _compute_actions(self, traj_data, curr_time, goal_time):
        ...
        # # Handle datasets where yaw elements are 1D arrays (e.g., go_stanford)
        # # Check if yaw dtype is object or if yaw[0] is an array
        # if yaw.dtype == object:
        #     yaw = np.array([np.asarray(y).item() if isinstance(np.asarray(y), np.ndarray) and np.asarray(y).ndim > 0 else np.asarray(y).item() for y in yaw])
        # elif yaw.size > 0:
        #     first_yaw = np.asarray(yaw[0])
        #     if isinstance(first_yaw, np.ndarray) and first_yaw.ndim > 0:
        #         yaw = np.array([np.asarray(y).item() if isinstance(np.asarray(y), np.ndarray) and np.asarray(y).ndim > 0 else y for y in yaw])
        ...
        # Ensure yaw[0] is a scalar for yaw_rotmat
        curr_yaw = np.asarray(yaw[0])
        if curr_yaw.ndim > 0:
            curr_yaw = curr_yaw.item()

        waypoints_pos = to_local_coords(positions, positions[0], curr_yaw)
        waypoints_yaw = angle_difference(curr_yaw, yaw)
        actions = np.concatenate([waypoints_pos, waypoints_yaw.reshape(-1, 1)], axis=-1)
        actions = actions[1:]
        
        # Handle goal_yaw similarly
        goal_yaw_scalar = np.asarray(goal_yaw)
        if goal_yaw_scalar.ndim > 0:
            goal_yaw_scalar = goal_yaw_scalar.item()

        goal_pos = to_local_coords(goal_pos, positions[0], curr_yaw)
        goal_yaw_diff = angle_difference(curr_yaw, goal_yaw_scalar)
