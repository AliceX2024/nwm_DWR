...
OUTPUT = Path('/DATA/DATANAS2/xiaoyj25/projects/nwm/eval_results/ablation_significance_analysis.md')
...
METHOD_DIRS = {
    ...
    'temmeDWR_bank4096': Path('/DATA/DATANAS2/xiaoyj25/projects/nwm/eval_results/multi_run_4datasets_clip04_CDiTXL_temmeDWR_bank4096_eval_0001600'),
    'temmeDWR_bank1024': Path('/DATA/DATANAS2/xiaoyj25/projects/nwm/eval_results/multi_run_4datasets_clip04_CDiTXL_temmeDWR_bank1024_eval_0001600'),
    'temmeDWR_bank512': Path('/DATA/DATANAS2/xiaoyj25/projects/nwm/eval_results/multi_run_4datasets_clip04_CDiTXL_temmeDWR_bank512_eval_0001600'),
...
DISPLAY_NAMES = {
    ...
    'temmeDWR_bank4096': 'temmeDWR(bank4096)',
    'temmeDWR_bank1024': 'temmeDWR(bank1024)',
    'temmeDWR_bank512': 'temmeDWR(bank512)',
    'latent_only_bank0': 'latent_only(bank0)',
}

ABLATIONS = ['temmeDWR_bank2048', 'temmeDWR_bank4096', 'temmeDWR_bank1024', 'temmeDWR_bank512', 'latent_only_bank0']
...
lines = []
lines.append('# 消融实验：4个数据集 × 4个时间点 × 指标 对比分析')
lines.append('')
...
lines.append('- 比较对象包含 baseline、noDWR、temmeDWR(bank2048) 以及不同 bank size / latent-only 变体。')
...
for dataset in sorted(structured):
    ...
    for time in TIMES:
        ...
        lines.append('')
        lines.append('| 指标 | baseline | noDWR | bank2048 | bank4096 | bank1024 | bank512 | latent_only(bank0) | 结论 |')
        lines.append('|---|---:|---:|---:|---:|---:|---:|---:|---|')
        for metric in sorted(structured[dataset][time]):
            row = structured[dataset][time][metric]
            all_methods = ['baseline', 'noDWR', 'temmeDWR_bank2048', 'temmeDWR_bank4096', 'temmeDWR_bank1024', 'temmeDWR_bank512', 'latent_only_bank0']
            ...
            lines.append('| {} | {} | {} | {} | {} | {} | {} | {} | {} |'.format(
                ...
                fmt_pm(row['temmeDWR_bank4096']['mean'], row['temmeDWR_bank4096']['std']),
                fmt_pm(row['temmeDWR_bank1024']['mean'], row['temmeDWR_bank1024']['std']),
                fmt_pm(row['temmeDWR_bank512']['mean'], row['temmeDWR_bank512']['std']),
...
for dataset, times in structured.items():
    for time, metrics in times.items():
        for metric, row in metrics.items():
            ...
            for method in ['temmeDWR_bank4096', 'temmeDWR_bank1024', 'temmeDWR_bank512', 'latent_only_bank0']:
                cur = row[method]
