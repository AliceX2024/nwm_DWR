...
def run_infer(run_dir, args, run_seed):
    ...
    cmd = [
        ...
        "--eval_type", args.eval_type_single,
...
def run_eval(args, run_dir):
    ...
    isolated_nwm_infer.py constructs the output subdirectory as:
        os.path.join(output_dir,
                     os.path.basename(exp).split('.')[0]   <- config filename stem
                     + (("_" + ckp) if ckp != '0100000' else ""))   <- checkpoint suffix
    e.g. for --exp config/4datasets_clip04_CDiTXL_temmeDWR_bank512_eval.yaml
              --ckp 0001600:
        "4datasets_clip04_CDiTXL_temmeDWR_bank512_eval_0001600"

    isolated_nwm_eval.py reads from:
        exp_dir / {dataset} / eval_type / id_*/

    GT images are pre-existing at gt_dir/{dataset}/eval_type/id_*/  (NO checkpoint sublayer).
    Fresh experiment runs live at run_dir/{exp_stem}/{dataset}/eval_type/id_*/.
    """
    exp_stem = Path(args.exp).stem
    if args.ckp != "0100000":
        exp_stem = f"{exp_stem}_{args.ckp}"

    if args.gt:
        # GT images have no checkpoint sublayer: eval reads directly under gt_dir
        eval_exp_dir = args.gt_dir
    else:
        # Fresh run: infer created {exp_stem} sublayer; eval reads from one level above dataset
        eval_exp_dir = str(Path(run_dir) / exp_stem)
...
def main(args):
    ...
    if args.gt:
        # GT images live at gt_dir/{dataset}/eval_type/ with NO {run_name}/{ckp} sublayer.
        # eval writes JSON to gt_dir/{dataset}/{dataset}_{eval_type}.json
        first_dataset = args.datasets.split(",")[0]
        gt_json = Path(args.gt_dir) / first_dataset / f"{first_dataset}_{args.eval_type_single}.json"
        if not gt_json.exists():
            # No JSON yet — we still skip infer (GT has no randomness); run eval directly.
            # eval_exp_dir = args.gt_dir because GT images have no sublayer.
            print(f"[INFO] GT JSON not found at {gt_json}, running eval with existing GT images...")
            print(f"[INFO] GT images expected at: {args.gt_dir}/{first_dataset}/{args.eval_type_single}/id_*/")
            gt_run_dir = args.gt_dir          # eval_exp_dir = gt_dir (no sublayer)
            run_eval(args, gt_run_dir)
        else:
            print(f"[INFO] GT JSON found at {gt_json}, skipping.")
...
if __name__ == "__main__":
    ...
    # Keep full comma-separated string for isolated_nwm_eval.py --eval_types
    # but also derive the single eval_type for isolated_nwm_infer.py --eval_type
    parsed.eval_type_single = parsed.eval_type.split(",")[0]
