...
OUTPUT = Path('/DATA/DATANAS2/xiaoyj25/projects/nwm/eval_results/ablation_random_reweight_significance_analysis.md')
...
ABLATIONS = ['temmeDWR_bank2048', 'temmeDWR_random_reweight']
COMPARE_TO = ['baseline', 'noDWR', 'temmeDWR_bank2048']
ALL_METHODS = ['baseline', 'noDWR', 'temmeDWR_bank2048', 'temmeDWR_random_reweight']
HIGHER_BETTER = {'psnr'}
...
lines = []
lines.append('# 消融实验：4个数据集 × 5个时间点 × 指标 对比分析（random reweight）')
lines.append('')
...
lines.append('- 比较对象包含 baseline、noDWR、temmeDWR(bank2048) 以及 random reweight 变体。')
...
for dataset in sorted(structured):
    ...
    for time in TIMES:
        ...
        lines.append('')
        lines.append('| 指标 | baseline | noDWR | bank2048 | random_reweight | 结论 |')
        lines.append('|---|---:|---:|---:|---:|---|')
...
for dataset, times in structured.items():
    for time, metrics in times.items():
        for metric, row in metrics.items():
            ...
            cur = row['temmeDWR_random_reweight']
            ...
            records.append((effect, dataset, time, metric, delta, p))
...
lines.append('')
lines.append('| 数据集 | 时间 | 指标 | 相对 bank2048 改变量 | p 值 | 显著性 |')
lines.append('|---|---:|---|---:|---:|:---:|')
for effect, dataset, time, metric, delta, p in records[:12]:
    lines.append('| {} | {} | {} | {} | {} | {} |'.format(
